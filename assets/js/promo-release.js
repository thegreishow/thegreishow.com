(function(){
  ...
  async function renderCatalog(){
    const {data,error}=await db.from('owner_releases').select('*').eq('status','published').order('featured',{ascending:false}).order('release_date',{ascending:false}).order('title');
    if(error||!data){showMissing('The promo library could not be loaded.');return;}
    document.title='Official Promo Library | The Grei Show';
    $('release-grid').innerHTML=data.map(cardMarkup).join('');
    document.querySelectorAll('.release-art img').forEach(wireImageFallback);
    $('loading').hidden=true;$('catalog').hidden=false;
  }

  function cardMarkup(r){
    const image=displayImage(r.artwork_url);
    return `<a class="release-card" href="/promo/${escAttr(r.slug)}/"><div class="release-art${image?'':' fallback'}" data-title="${escAttr(r.title)}">${image?`<img src="${escAttr(image)}" alt="${escAttr(r.title)} artwork" loading="lazy" data-drive-id="${escAttr(driveId(r.artwork_url)||'')}">`:''}</div><div class="release-copy"><h2>${esc(r.title)}</h2><div class="release-meta"><span>${esc(r.release_type||'Release')}</span><span>${esc(r.genre||'The Grei Show')}</span></div></div></a>`;
  }
  ...
  function renderDetail(r){
    ...
    if(image){
      $('cover').src=image;$('cover').alt=`${title} artwork`;$('backdrop').src=image;
      $('cover').dataset.driveId=driveId(r.artwork_url)||'';$('backdrop').dataset.driveId=driveId(r.artwork_url)||'';
      wireImageFallback($('cover'));wireImageFallback($('backdrop'));
    }else{
  ...
  function getSlug(){const parts=location.pathname.split('/').filter(Boolean);const i=parts.indexOf('promo');return i>=0&&parts[i+1]?decodeURIComponent(parts[i+1]):new URLSearchParams(location.search).get('slug')||'';}
  function driveId(url){if(!url)return'';const s=String(url);return(s.match(/[?&]id=([\w-]+)/)||s.match(/\/d\/([\w-]+)/)||s.match(/folders\/([\w-]+)/)||[])[1]||'';}
  function displayImage(url){if(!url)return'';const id=driveId(url);if(id&&String(url).includes('drive.google.com'))return`https://lh3.googleusercontent.com/d/${id}=w1600`;return String(url);}
  ...
  function wireImageFallback(img){img.addEventListener('error',()=>{const id=img.dataset.driveId;if(id&&!img.dataset.retried){img.dataset.retried='1';img.src=`https://drive.google.com/thumbnail?id=${id}&sz=w1600`;return;}const box=img.closest('.release-art,.cover');if(box){box.classList.add('fallback');if(!box.dataset.title)box.dataset.title=img.alt.replace(/ artwork$/,'');}img.remove();},{once:false});}
