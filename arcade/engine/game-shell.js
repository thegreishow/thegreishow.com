(() => {
  ...
  // === SHARED SOUND SYSTEM ===
  const SOUND_KEY = 'grei_arcade_sound';
  ...
  style.textContent = `
    ...
    .grei-gamebar a,.grei-gamebar button{display:inline-flex;align-items:center;justify-content:center;min-height:40px;padding:0 13px;border:1px solid rgba(255,255,255,.16);border-radius:999px;background:rgba(255,255,255,.07);color:#fff;text-decoration:none;font:800 12px/1 system-ui;letter-spacing:.04em;cursor:pointer;white-space:nowrap}
    .grei-gamebar a:focus-visible,.grei-gamebar button:focus-visible,.grei-pause-screen button:focus-visible{outline:3px solid #72ffb4;outline-offset:2px}
    ...
    @media(max-width:560px){.grei-gamebar strong{display:none}.grei-gamebar{gap:5px;padding-left:6px;padding-right:6px}.grei-gamebar-actions{gap:5px;min-width:0}.grei-gamebar a,.grei-gamebar button{min-height:44px;padding:0 10px}.grei-gamebar [data-grei-fullscreen]{width:44px;padding:0;font-size:0}.grei-gamebar [data-grei-fullscreen]::after{content:'⛶';font-size:19px}.grei-gamebar-spacer{height:62px}}
  `;
  ...
  bar.innerHTML = `
    ...
    <div class="grei-gamebar-actions">
      <button class="sound-btn" type="button" data-grei-sound aria-label="Turn Arcade sounds ${soundEnabled ? 'off' : 'on'}" aria-pressed="${soundEnabled}">${soundEnabled ? '🔊' : '🔇'}</button>
      <button type="button" data-grei-pause aria-label="Pause game" aria-pressed="false">Pause</button>
      <button type="button" data-grei-fullscreen aria-label="Enter full screen">Full screen</button>
    </div>`;
  ...
  pauseScreen.setAttribute('aria-label', 'Game paused');
  pauseScreen.setAttribute('aria-hidden', 'true');
  pauseScreen.innerHTML = '<div><h2>PAUSED</h2><p>Tap Resume or press P to continue.</p></div>';
  ...
  soundBtn.addEventListener('click', () => {
    ...
    soundBtn.setAttribute('aria-label', `Turn Arcade sounds ${enabled ? 'off' : 'on'}`);
  });
  ...
  function setPaused(next) {
    ...
    pauseScreen.setAttribute('aria-hidden', String(!paused));
  ...
  const updateFullscreenButton = () => {
    const active = Boolean(nativeFullscreenElement() || immersive);
  ...
  window.addEventListener('keydown', event => {
    ...
    if (event.key.toLowerCase() === 'p') { event.preventDefault(); setPaused(!paused); }
  ...
  window.addEventListener('DOMContentLoaded', () => {
    if (gameName !== 'Rasta Runner') return;
