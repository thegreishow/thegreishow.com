...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (event.event_type === "CHECKOUT.ORDER.APPROVED") {
      ...
      const { data: musicSupport } = payment ? { data: null } : await admin.from("music_support_payments").select("id,status").eq("paypal_order_id", orderId).maybeSingle();
      const payable = payment || musicSupport;
      if (payable && !["paid","completed"].includes(payable.status)) {
        const response = await fetch(`${paypalBaseUrl()}/v2/checkout/orders/${encodeURIComponent(orderId)}/capture`, { method:"POST", headers:{ Authorization:`Bearer ${accessToken}`, "Content-Type":"application/json", "PayPal-Request-Id":`${payment?"white-line":"music-support"}-capture-${payable.id}`, Prefer:"return=representation" }, body:"{}" });
...
async function updateCaptureStatus(admin:any,event:any,status:string) {
  ...
  if(!payment){
    const {data:musicSupport,error:supportError}=await admin.from("music_support_payments").select("id").eq("paypal_order_id",orderId).maybeSingle();
    if(supportError)throw supportError;if(!musicSupport)return;
    const supportResult=await admin.from("music_support_payments").update({status,provider_payload:{webhook_event_id:event.id||null,capture_status:event.resource?.status||status},updated_at:new Date().toISOString()}).eq("id",musicSupport.id);
    if(supportResult.error)throw supportResult.error;return;
  }
